...
def generate_reply(prompt, use_google_ai=True, use_file_reply=False, language="id"):
    """Generates a reply while avoiding duplicates if using Google Gemini AI"""

    global last_ai_response  # Use a global variable to access it throughout the session

    if use_file_reply:
        log_message("💬 Using messages from file as replies.")
    ...
    if use_google_ai:
        ...
        if language == "en":
            ...
            ai_prompt = f"{prompt}\n\nGive only one sentence in Jakarta slang like a natural conversation and do not use any symbols."

        url = f'https://generativelanguage.googleapis.com/v1beta/models/gemini-1.5-flash-latest:generateContent?key={google_api_key}'
        headers = {'Content-Type': 'application/json'}
        data = {'contents': [{'parts': [{'text': ai_prompt}]}]}

        for attempt in range(3):  # Try up to 3 times if AI repeats the same response
            try:
        ...
        return {"candidates": [{"content": {"parts": [{"text": last_ai_response or 'Sorry, unable to respond.'}]}}]}

    else:
        return {"candidates": [{"content": {"parts": [{"text": get_random_message()}]}}]}

def get_random_message():
    """Fetches a random message from pesan.txt file"""
    try:
...
def send_message(channel_id, message_text, reply_to=None, reply_mode=True):
    """Sends a message to Discord, with or without replying"""
    headers = {
...
def auto_reply(channel_id, read_delay, reply_delay, use_google_ai, use_file_reply, language, reply_mode):
    """Function for auto-reply on Discord while avoiding AI duplicates"""
            ...
            if response.status_code == 200:
                ...
                if len(messages) > 0:
                    ...
                    if (last_message_id is None or int(message_id) > int(last_message_id)) and author_id != bot_user_id and message_type != 8:
                        ...
                        response_text = result['candidates'][0]['content']['parts'][0]['text'] if result else "Sorry, unable to respond."
...
if __name__ == "__main__":
    use_reply = input("Do you want to enable auto-reply? (y/n): ").lower() == 'y'
    channel_id = input("Enter the channel ID: ")

    if use_reply:
        use_google_ai = input("Use Google Gemini AI for replies? (y/n): ").lower() == 'y'
        use_file_reply = input("Use messages from pesan.txt? (y/n): ").lower() == 'y'
        reply_mode = input("Do you want to reply to messages or just send them? (reply/send): ").lower() == 'reply'
        language_choice = input("Choose the reply language (id/en): ").lower()

        if language_choice not in ["id", "en"]:
            log_message("⚠️ Invalid language, defaulting to Indonesian.")
            language_choice = "id"

        read_delay = int(input("Set delay for reading new messages (in seconds): "))
        reply_delay = int(input("Set delay for replying (in seconds): "))

        log_message(f"✅ Reply mode {'enabled' if reply_mode else 'non-reply'} in {'Indonesian' if language_choice == 'id' else 'English'}...")
        auto_reply(channel_id, read_delay, reply_delay, use_google_ai, use_file_reply, language_choice, reply_mode)

    else:
        send_interval = int(input("Set message send interval (in seconds): "))
